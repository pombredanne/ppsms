...
function post_sms($postdb){//将需要发送的短信存入等待发送列表
		...
		$cost_num=count($postdb[receiver])*$sms_num;//发送短信总数量
		...
		if($postdb[send_time])$state="等待发送";
		else{
			$state="发送成功";
			$postdb[send_time]=$timestamp;
			if($send_sms_result=send_sms(implode(',',$postdb[receiver]),$postdb[message]))return $send_sms_result;//发送
		}
		//插入到短信列表
...
function post_sms_do($postdb){//简单发送和通讯录发送短信逻辑
		...
		if($postdb[message]=="")die('{"name":"postdb[message]","tips":"请输入发送的短信内容"}');
		if($postdb[time]=="true"){//若为定时短信类型
			if(!$postdb[time_date])die('{"name":"postdb[time_date]","tips":"请设置定时发送的时间"}');
			//判断定时短信必须大于当前时间
			$postdb[send_time]=strtotime("$postdb[time_date] $postdb[time_hh]:$postdb[time_ii]:00");
			if($postdb[send_time]<$timestamp)die('{"name":"postdb[time_date]","tips":"您设置定时发送的时间必须大于当前时间"}');
			//将定时发送的短信存在等待发送列表
...
function get_receiver_array($receiver_temp){//校验过滤并获取接收号码，参数为数组对象
		foreach($receiver_temp AS $key=>$value){
				...
				if(!preg_match("/^13[0-9]{9}$|15[0-9]{9}$|18[0-9]{9}$/",$value)){//规则：所有13、15、18开头的所有11位数字
						die('{"name":"postdb[receiver]","tips":"接收号码：'.$value.' 不符合号码规则"}');
				}
				$receiver_arr[]=$value;
		}
		if(!$receiver_arr)die('{"name":"postdb[receiver]","tips":"请填写正确的手机号码"}');
		return $receiver_arr;
}
