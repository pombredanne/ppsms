...
function html($html,$tpl=''){
	...
	echo "==".ROOT_PATH."template/default/".$html.".htm"."==";
}
...
*页面跳转函数
**/
function refresh2($url,$msg="网页正在跳转当中,请稍候...",$time=1){
	global $webdb;
	if($time==0){
		header("location:$url");
	}else{
		require(ROOT_PATH."inc/head.php");
		require(ROOT_PATH."template/default/refresh2.htm");
...
*确认页面函数
**/
function confirmm($showerrMsg,$url,$nav_arr){
	require_once(ROOT_PATH."data/level.php");
		extract($GLOBALS);
		$nav_arr=explode(",",$nav_arr);
		foreach($nav_arr as $key=>$value){
			$nav.="<a href='#'>$value</a> <img src='$webdb[www_url]/images/location_arrow.gif' /> ";
		}
		require(ROOT_PATH."inc/head.php");
		require(ROOT_PATH."template/default/confirmm.htm");
		require(ROOT_PATH."inc/foot.php");
		$content=ob_get_contents();
		ob_end_clean();
		ob_start();
...
function delete_attachment($uid,$str){
	...
	foreach( $filedb AS $key=>$value){
		...
		if($detail[0]&&$detail[0]==$uid){
			...
			//if($rs=$db->get_one("SELECT * FROM {$pre}upfile WHERE url='$name'")){
				//if($rs[num]>1){
				//	$db->query("UPDATE `{$pre}upfile` SET `num`=`num`-1 WHERE url='$name'");
				//	continue;
				//}
				$db->query("DELETE FROM `{$pre}upfile` WHERE url='$name'");
			//}
...
function player($url,$width=400,$height=300,$autostart='false',$force=''){
	global $webdb;
	//$urlstring=str_replace('+','%2B',mymd5($url));
